...
class DataService:
    ...
    async def add_entry(self, raw_text: str) -> dict:
        """
        Create a new entry with real sentiment extraction via Groq LLM,
    ...
    def _extract_sentiment(self, raw_text: str) -> tuple:
        """Run Groq LLM extraction. Returns (score, tags, triggers)."""
        ...
        try:
            print(f"[LLM] Calling Groq for extraction on text: {raw_text}")
            ...
            print(f"[LLM] Groq extraction successful: score={sentiment}, tags={extracted.tags}, triggers={extracted.triggers}")
